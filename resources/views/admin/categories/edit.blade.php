<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-800 dark:text-white mt-2">Edit Kategori: {{ $category->name }}</h1>
    </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select name="type" required
                    class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <option value="audio" {{ $category->type == 'audio' ? 'selected' : '' }}>Audio</option>
            ...
            <div class="mb-6">
                ...
                <select name="marhalah"
                    class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @for($i = 1; $i <= 6; $i++)
                        <option value="{{ $i }}" {{ old('marhalah', $category->marhalah) == $i ? 'selected' : '' }}>Marhalah
                            {{ $i }}</option>
                    @endfor
                </select>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $category->description) }}</textarea>
            </div>
